...
std::vector<LContainer> Execute(FILE* fin) {
	...
	while((fileChar = fgetc(fin)) != EOF) // Read from a file a character at a time. FIXME: EOF doesn't allow the last Token to be recorded- logic error
	{
		
		if (fileChar != '\n' && fileChar != ' ' && fileChar != '=') // If we are not at the end of the file or at a space or an equal
		{
			stringFlag          = true;     //We are now starting reading a string
